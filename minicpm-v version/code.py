...
def query_minicpm_with_image(image, query):
    try:
        # Save the image to a temporary file
        temp_image_path = tempfile.NamedTemporaryFile(delete=False, suffix=".png").name
        image.save(temp_image_path)

        # Pass the image path and query to the minicpm-v model
        response = ollama.chat(
            model='minicpm-v',  # Use minicpm-v model
            messages=[
                {
                    'role': 'user',
                    'content': query,
        ...
        st.error(f"An error occurred while querying MiniCPM: {e}")
        return "Error occurred."


def query_ollama_with_text(query, context):
    try:
        prompt = (
            f"Answer the following question *only* using the provided context from the PDF. "
            f"Do not use any outside knowledge or make assumptions beyond this context.\n\n"
            f"Context: {context}\n\n"
            f"Question: {query}"
        )
        response = ollama.chat(
...
st.header("Image Upload and Query")

uploaded_image = st.file_uploader("Drag and drop an image (PNG, JPG, JPEG):", type=["png", "jpg", "jpeg"])
...
if uploaded_image:
    try:
        ...
        # Ask for a query about the image
        st.subheader("Ask about the uploaded image")
        image_query = st.text_input("Enter your query for the uploaded image:")

        if image_query:
            with st.spinner("Querying MiniCPM with the image..."):
                response = query_minicpm_with_image(pil_image, image_query)
...
if st.session_state.vectorstore:
    ...
    # Text Query for PDF
    st.write("### Query the Text in the PDF")
    text_query = st.text_area("Enter your query for the text content in the PDF:")

    if text_query:
        # Retrieve similar text context from PDF using the vectorstore
        docs = st.session_state.vectorstore.similarity_search(text_query, k=3)
        context = "\n\n".join([doc.page_content for doc in docs])

        with st.spinner("Querying MiniCPM for text..."):
            response = query_ollama_with_text(text_query, context)
            st.write("MiniCPM's response:", response)